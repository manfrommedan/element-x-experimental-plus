...
class DefaultUrlPreviewService(
    ...
    // Access-ordered, size-bounded cache: a long-lived session that scrolls through many links
    // must not accumulate previews forever. Successful lookups (including the "no preview" result)
    // are cached; failures are not, so they can be retried. Only touched under [cacheMutex].
    private val cache = object : LinkedHashMap<String, UrlPreviewData?>(16, 0.75f, true) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, UrlPreviewData?>): Boolean = size > MAX_CACHE_ENTRIES
    }
    private val cacheMutex = Mutex()

    // One lock per in-flight URL so that the same link appearing in many messages triggers a single
    // network request instead of a burst of identical ones. Entries are dropped once the fetch ends.
    private val inFlightLocks = mutableMapOf<String, Mutex>()
    private val inFlightLocksGuard = Mutex()

    override suspend fun getPreview(url: String): Result<UrlPreviewData?> {
        readFromCache(url)?.let { return it }

        val perUrlLock = inFlightLocksGuard.withLock { inFlightLocks.getOrPut(url) { Mutex() } }
        try {
            return perUrlLock.withLock {
                // Another caller may have populated the cache while we waited for the lock.
                readFromCache(url)?.let { return@withLock it }
                fetchPreview(url).also { result ->
                    if (result.isSuccess) {
                        cacheMutex.withLock { cache[url] = result.getOrNull() }
                    }
                }
            }
        } finally {
            inFlightLocksGuard.withLock { inFlightLocks.remove(url) }
        }
    }

    private suspend fun readFromCache(url: String): Result<UrlPreviewData?>? =
        cacheMutex.withLock { if (cache.containsKey(url)) Result.success(cache[url]) else null }
