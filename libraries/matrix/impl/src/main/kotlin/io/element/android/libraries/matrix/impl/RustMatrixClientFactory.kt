...
class RustMatrixClientFactory(
    ...
    suspend fun create(sessionData: SessionData): RustMatrixClient = withContext(coroutineDispatchers.io) {
        ...
        // The index is encrypted with the client secret (see getBaseClientBuilder), so a session
        // without one would get a plaintext index of every message body on disk. Sessions predating
        // passphrase support are the realistic case; for them search stays off rather than trading
        // the store's encryption for it.
        val isMessageSearchAvailable = featureFlagService.isFeatureEnabled(FeatureFlags.MessageSearch) &&
            clientSecret != null
        if (clientSecret == null && featureFlagService.isFeatureEnabled(FeatureFlags.MessageSearch)) {
            Timber.tag("RustMatrixClient").w("Message search is enabled but this session has no secret to encrypt the index with, leaving it off")
        }
...
/** Records that the search index was created before anything reached the event cache store. */
